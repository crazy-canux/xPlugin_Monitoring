...

{
  ...
  sub init_databases {
    ...
    if (($params{mode} =~ /server::database::listdatabases/) ||
        ...
        ($params{mode} =~ /server::database::databaseused/) ||
  ...
  if (! $self->{nagios_level}) {
	...
	printf "%s\n", $_->{logicalfilename};
      ...
      $self->add_perfdata(sprintf "%s_transactions_per_sec=%.4f;%s;%s",
          $self->{name}, $self->{transactions_per_sec},
          $self->{warningrange}, $self->{criticalrange});
    } elsif ($params{mode} =~ /server::database::databaseused/) {
      # Used
      if (($self->{warningrange} && $self->{warningrange} !~ /^\d+[\.\d]*$/) ||
          ($self->{criticalrange} && $self->{criticalrange} !~ /^\d+[\.\d]*$/)) {
        $self->add_nagios_unknown("you want an alert if used space is _inferior_ a threshold????");
        return;
      }
      if (! $params{units}) {
        $params{units} = "MB";
      ...
      $self->{warningrange} ||= $self->{max_mb} - 5;
      $self->{criticalrange} ||= $self->{max_mb} - 2;
      $self->add_nagios(
          $self->check_thresholds($self->{used_mb}),
                 sprintf("database %s has %.2f%s used space", $self->{name},
                  $self->{used_mb} / $factor, $params{units})
      );
      $self->add_perfdata(sprintf "\'db_%s_used\'=%.2f%s;%.2f;%.2f;0;%.2f",
            lc $self->{name},
            $self->{used_mb} / $factor, $params{units},
            $self->{warningrange} / $factor,
            $self->{criticalrange} / $factor,
            $self->{max_mb} / $factor);
    } elsif ($params{mode} =~ /server::database::databasefree/) {
      # ->percent_free
      # ->free
