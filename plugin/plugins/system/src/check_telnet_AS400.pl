...
### FUNC: hostmatch
sub hostmatch {
	if (!$telnet->waitfor(Match => '/'. $match .'/', Errmode => "return")) {
		print "CRITICAL: Login page is not available (Search for: $match).\n";
		exit 2;
	} else {
		print "OK: login page is available (Search for: $match).\n";
		exit 0;
	}
	$telnet->close;
} #/sub
