...
if  [ $RES = 0 ]; then
    ...
    UPTHOUR=$(expr $UPTMIN / 60 )
    UPTDAY=$(expr $UPTMIN / 60 / 24 )
    if [ $UPTHOUR -gt $CRIT ]; then  
        echo CRITICAL: Systemuptime $UPTHOUR hours.
        exit 2
    fi

    if [ $UPTHOUR -gt $WARN ]; then  
        echo WARNING: Systemuptime $UPTHOUR hours.
        exit 1
    fi

    if [ $UPTHOUR -le $WARN ]; then
        echo OK: Systemuptime $UPTHOUR hours
        exit 0
    fi
fi
