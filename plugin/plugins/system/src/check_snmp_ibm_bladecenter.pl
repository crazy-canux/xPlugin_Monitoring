...
use strict;
require 5.6.0;
use lib qw( /usr/lib/canux/plugins );
use utils qw(%ERRORS $TIMEOUT &print_revision &support &usage);
use Net::SNMP;
use Getopt::Long;
use vars qw/$exit $message $opt_version $opt_timeout $opt_help $opt_command $opt_host $opt_community $opt_verbose 
$opt_warning $opt_critical $opt_port $opt_mountpoint $snmp_session $PROGNAME $TIMEOUT $test_details $test_name $test_num $output/;
...
$output = "$res $test_name ($data) $data_text | $test_name=$data;";
if (defined($opt_warning) && defined($opt_critical)) {
	$output .= "$opt_warning;$opt_critical;";
}
$output .= "\n";
print $output;
